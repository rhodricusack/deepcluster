...
df_aoa=pd.read_json('/home/rhodricusack/deepcluster_analysis/linearclass_v3_aoa.json')
...
for convkey,convgrp in df_aoa.groupby('conv'):
    ...
    for nodekey,nodegrp in convgrp.groupby('node'):
        ...
        p=curve_fit(learningcurve,epoch,prec5,p0=[100,0], maxfev = 10000,bounds=((0,-20),(100,10))) 
        lc[convkey-1][nodekey]={'A':p[0][0],'k':p[0][1]}
        ...
        prec5_fits=[learningcurve(s,p[0][0],p[0][1]) for s in epoch]
        colind=int(((nodegrp['aoa_rank'].iloc[0]-aoamin)/aoarange)*255)
