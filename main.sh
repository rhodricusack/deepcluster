...
# For SPOT instances 
CHECKPOINTPATH="${EXP}/checkpoints"
...
${PYTHON} main.py ${DIR} --exp ${EXP} --arch ${ARCH} \
  --lr ${LR} --wd ${WD} --k ${K} --sobel --verbose --workers ${WORKERS} --checkpoints 5000 --resume ${CHECKPOINTPATH}/$RESUME --resume_fallback ${CHECKPOINTPATH}/$RESUME_FALLBACK
